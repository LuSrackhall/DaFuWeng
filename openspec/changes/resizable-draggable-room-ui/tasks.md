...
- [x] 6.17 Fix drag stutter and viewport-resize disappearing-window bugs in both floating surfaces: defer `rndRef.updateSize/updatePosition` inside `useEffect` callbacks to prevent the `setState + imperative Rnd update` race condition that caused visual jumping; guard snap-correction `updatePosition/updateSize` calls in `onDragStop`/`onResizeStop` so they only fire when snap actually changed the position (avoiding redundant position battles with Rnd's own internal drag tracking). All product features from 6.12 and 6.13 (HUD, snap guides, recovery chip, feedback animations) are preserved.
